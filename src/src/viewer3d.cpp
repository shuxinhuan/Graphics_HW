...
int Viewer3D::launchInit() {
	...
	mNGui->addButton("Update", [&]() { for (int i = 0; i < 100; i++) { smoothMesh(); }  });
	...
	//mNGui->addVariable("Rotation Invariant", mRotInva);
...
void Viewer3D::openDialogLoadMesh() {
	...
	//std::vector< HEdge* > tmpVer = mMesh->boundaryEdges();
	//std::cout << tmpVer[0]->flag() <<" " << tmpVer[2]->flag();
}
